	...
	for (;;) {
		...
		case KVM_EXIT_IO:
			...
			if(vcpu->kvm_run->io.direction == KVM_EXIT_IO_OUT && vcpu->kvm_run->io.port == 0xEB)
				{
					struct kvm_translation kvm_translate;
					char *p = (char *)vcpu->kvm_run;
					uint32_t mem;
					memcpy(&mem,p + vcpu->kvm_run->io.data_offset,vcpu->kvm_run->io.size);
					kvm_translate.linear_address = mem;
					if(ioctl(vcpu->fd,KVM_TRANSLATE,&kvm_translate)<0)
					{
						perror("Error in translating GVA");
						exit(1);
					}
					size_t len = 0;
					while(vm->mem[kvm_translate.physical_address+len]!=0)
					{
						++len;
					}
					// fwrite(&vm->mem[kvm_translate.physical_address],sizeof(char*),len,stdout);
					printf("%s",vm->mem+kvm_translate.physical_address);
			...
			{
					char *p = (char *)vcpu->kvm_run;
					p[vcpu->kvm_run->io.data_offset] = num_exits;
					continue;
			}
			if (vcpu->kvm_run->io.direction == KVM_EXIT_IO_OUT && vcpu->kvm_run->io.port == 0xEC)
			{
					...
					if(kvm_translate.valid==1)
					{
						hv_addr = (uintptr_t)(vm->mem+kvm_translate.physical_address);
						continue;
					}
					else
					{	
			...
			if (vcpu->kvm_run->io.direction == KVM_EXIT_IO_IN && vcpu->kvm_run->io.port == 0xEC)
			{
					...
					char *p = (char *)vcpu->kvm_run;
					*(uint32_t *)(p + vcpu->kvm_run->io.data_offset) = hv_addr;
					continue;
			}
			if (vcpu->kvm_run->io.direction == KVM_EXIT_IO_IN && vcpu->kvm_run->io.port == 0xED)
			{
					...
					char *p = (char *)vcpu->kvm_run;
					if(ioctl(vm->fd,KVM_GET_CLOCK,&kvm_clock)<0)
					{
						perror("Error in fetching clock time");
						exit(1);
					}
					
					*(uint32_t *)(p + vcpu->kvm_run->io.data_offset) = kvm_clock.clock;
					continue;
			}
